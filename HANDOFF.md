...
**Build:** 61 (iOS Xcode) / **web now hosted on Vercel** → https://drift-one-jade.vercel.app (auto-deploys on push to `main`; BYOK, no key in the bundle)
**Status:** This session (Jun 21–22, entry 178): **Public Vercel launch + free "on us" demo.** Connected the repo to Vercel (Hobby) and verified the BYOK security posture (no secrets committed, no key in the prod bundle, strong build-time CSP). Built a fully keyless "free examples" experience so visitors can taste Drift with ZERO API calls: the 4 welcome example prompts ship pre-written answers, each pre-marked term carries pre-written content for ALL six lenses (Drift / Simplify / Deep dive / Stress test / Evidence / Connect-with-bridges), examples persist on the welcome screen until the user brings their own key, and Settings no longer auto-opens on a missing key. Fixed two tooltip jump/clipping bugs (welcome-card capability tooltip + the highlight selection menu) and made the selection menu position from its real measured height so it never clips at any selection position. Also PR housekeeping: merged 6 green Dependabot/CI PRs + upgraded to ESLint 10; held onboarding PR #31 (duplicates the free-examples work). (full detail in section 178 below). Prior session (Jun 12, entry 177): **Challenge → Second opinion rename + evidence-driven prompts — Challenge renamed to Second opinion (independent model gives its verdict, agrees/agrees-with-caveats/disagrees, never manufactures disagreement), Steelman removed, Evidence upgraded to request high-quality citations (prioritizes Google Search grounding, peer-reviewed papers + meta-analyses over blogs, specific citations with year/journal/authors). Also: fixed duplicate "Second opinion on this" label in sidebar, fixed numbered-list layout bug (inline span wrapping was orphaning the list marker).** (full detail in section 177 below). Prior session (Jun 11, entry 176): **Welcome-screen polish** (full detail in section 176 below). Prior session (Jun 11, entry 175): **Launch prep — Arabic RTL + centralized language detection, prod API-key-leak fix, App Store metadata + 5 dark screenshots** (full detail in section 175 below; open screenshot decisions listed there). Prior session (Jun 10, entry 174): **Cloud accounts + UI polish** — (1) Cloud accounts implementation (Phase 1–3 complete): `src/lib/cloudConfig.ts` master gate, `src/services/firebase.ts` lazy init, `src/services/auth.ts` Apple sign-in (native iOS + web popup), `src/services/cloudSync.ts` backup/restore with 5s debounce, `src/services/cloudKeyStrip.ts` pure key-removal module + 6 vitest tests (zero keys in uploads guaranteed), `src/services/cloudHooks.ts` change-bus, `src/store/authStore.ts` Zustand, `src/components/account/SignInSheet.tsx` + `AccountSection.tsx` glassmorphic UI with EASE_OUT_EXPO animations, settings integration. Dynamic imports ensure Firebase code never fetches when disabled. Playwright verified: blank env ⇒ no Account UI, no Firebase init; env-filled ⇒ Account renders on-brand. `package.json` Firebase 12.x + @capacitor-firebase/authentication 8.3, vitest devDep. Three feature branches ready for PR: `feature/cloud-auth`, `feature/cloud-sync`, `feature/cloud-ui` (consolidates into `feature/cloud-accounts`). Owner setup checklist in HANDOFF.md. (2) UI polish fixes: arc label "related by field" → "related" (two places: legend + hover tooltip in DriftKnowledgeGraph); removed header "reopen last drift" chip block (38 lines deleted from App.tsx); fixed sidebar blank-chat dedup (demo-trim + load-time pruning + createChat guard). (3) Build: tsc clean, production build 298.45 kB JS / 125.54 kB CSS (gzip 86.48 / 18.60 kB), cap sync clean, build 58→59. All code committed + pushed to `fix/sidebar-map-chip-polish`. Next: open 3 cloud PRs (pending `gh auth login`), owner Firebase setup, TestFlight build. Prior session (Jun 9 continued, entry 173): **Model-name label removed**. Prior (Jun 9 continued, entry 172): **Language fix + highlights polish + map lens colors**.

## ☁️ Cloud accounts + backup/restore (Jun 10, 2026 — branch stack off `main`)

**What:** OPTIONAL Firebase account (Apple sign-in only, v1) that backs up / restores all local data (chats, drifts, snippets, settings-minus-keys, theme) to Firestore at `users/{uid}/backup/current`. NOT real-time sync — cloud backup/restore with last-write-wins by `updatedAt`. **Completely invisible and inert until `.env` has all `VITE_FIREBASE_*` vars filled** (`isCloudEnabled()` in `src/lib/cloudConfig.ts` gates everything). Verified via `scripts/verify-cloud-disabled.mjs`: with blank env there is no Account UI, no Firebase init, no cloud network requests.

**Branches (stacked, each PRs into `feature/cloud-accounts`; web builds stay inert until env is filled):**
- `feature/cloud-accounts` — base: `.env.example` Firebase placeholders + `firestore.rules`
- `feature/cloud-auth` — `src/lib/cloudConfig.ts`, `src/services/firebase.ts` (lazy init), `src/services/auth.ts` (Apple; native plugin on iOS / popup on web; provider-agnostic so Google/email drop in later), `src/store/authStore.ts`
- `feature/cloud-sync` — `src/services/cloudSync.ts` (push/pull reuse `backup.ts`; debounced 5s auto-push; sign-in flow = pull-merge → converge push → auto-push), `cloudKeyStrip.ts` + 6 vitest tests (`npm test`) proving uploads contain ZERO API-key fields, `cloudHooks.ts` change bus. Existing-file touches: `db.ts` (3 emit lines), `main.tsx` (guarded dynamic import), `authStore` (sync fields)
- `feature/cloud-ui` — `src/components/account/AccountSection.tsx` + `SignInSheet.tsx` (glass sheet, framer-motion, pink/violet), lazy + gated render in `Settings.tsx` above Models; verify scripts

**Security invariants:** API keys (`geminiApiKey`, `openRouterApiKey`, preset `apiKey`) are deep-stripped + asserted absent before EVERY upload (`cloudKeyStrip.ts`); keys live in localStorage only. Firestore rules deny all cross-user access. Backup doc capped at ~950 KB (Firestore 1 MiB limit) — oversized pushes fail with a clear message pointing at local export.

### 🔑 OWNER MANUAL SETUP (required to turn the feature ON — nothing works until then)
1. **Firebase console** (console.firebase.google.com): create/select project → add a **Web app** → copy the config values into `.env` as `VITE_FIREBASE_*` (names in `.env.example`). Filling these is the feature flag.
2. **Firebase Auth:** Authentication → Sign-in method → enable **Apple**.
3. **Apple Developer:** create a **Services ID** for web sign-in (return URL: `https://<project>.firebaseapp.com/__/auth/handler`), and in Xcode add the **Sign in with Apple** capability to the App target.
4. **Firestore:** create the database, then deploy rules: `firebase deploy --only firestore:rules` (rules file already at `firestore.rules`).
5. **iOS only:** download `GoogleService-Info.plist` from Firebase into `ios/App/App/`, then `npx cap sync ios`. ⚠️ **Until the plist is in place, do NOT `cap sync` + build iOS from these branches** — the `@capacitor-firebase/authentication` pod expects it and the app can crash at launch without it. (Web builds are always safe.)
6. Smoke-test enabled mode: build with env set, then `node scripts/verify-cloud-enabled.mjs`, then a real Apple sign-in on web.
...
### 178. Public Vercel launch + free "on us" demo + tooltip fixes + PR housekeeping (Jun 21–22)

Web app went public on Vercel. All work committed + pushed to `main` across several commits; Vercel auto-deploys each push. iOS build unchanged (61).

**Vercel hosting + security review:**
- Connected the GitHub repo to Vercel (Hobby plan) → live at https://drift-one-jade.vercel.app. Pure Vite SPA, no router, so Vercel's defaults work (build `npm run build`, output `dist`) — **no `vercel.json` needed**, no required env vars.
- Security posture verified: `.env`/`.env.production` are tracked but intentionally empty (BYOK guards); no key ever committed (checked git history); the built `dist/` bundle has no Gemini key; Firebase env empty; strong build-time CSP (`default-src/script-src 'self'`, `object-src 'none'`, `frame-ancestors 'none'`, `connect-src https:`). The "Offline / Failed to connect" a keyless visitor saw was *by design* (BYOK) — which motivated the free-examples work below.

**Free "on us" examples — keyless visitors get a full taste, ZERO API calls:**
- `src/lib/freeExamples.ts` (NEW) — the 4 welcome example prompts each ship a pre-written root answer + a fixed set of dotted drift terms (same for everyone). Example chips ALWAYS resolve to their canned answer regardless of key state (a stale persisted key could otherwise route the tap to the real provider → "no API key").
- `src/lib/freeLensContent.ts` (NEW, ~104 KB, auto-generated then committed) — for each of the 12 pre-marked terms (3 per example: Rome `fiscal crisis`/`mercenaries`/`Crisis of the Third Century`; quantum `spooky action at a distance`/`correlation`/`faster than light`; philosophy `dukkha`/`attachment`/`dichotomy of control`; caffeine `adenosine`/`blood-brain barrier`/`half-life`), pre-written content for ALL built-in lenses: `simplify`/`research`/`challenge`/`evidence` prose, plus Connect (`cards` as `"type :: relationship :: concept"` strings + a `bridges` answer per card concept). Content authored by 4 parallel domain-expert agents, merged + validated (every card has a matching bridge). Helpers in `freeExamples.ts`: `getFreeExample`, `getFreeDriftAnswer`, `getFreeLensAnswer`, `getFreeConnectCards`, `getFreeConnectBridge`.
- `src/hooks/useMessageStream.ts` — intercepts the 4 example prompts and streams the canned root answer (chunked reveal ~6 chunks to avoid re-parsing growing markdown each token), attaching the fixed `suggestedHighlights` (the dotted terms).
- `src/hooks/useDriftMessageStream.ts` — when keyless and on the OPENING turn of a fresh thread on a pre-marked term, serves the right canned content by lens: prose lenses stream in chunks; Connect emits the raw card JSON (parsed into chips by `useConnectThreads`, with awaits forcing separate renders so React batching can't swallow the parse) then a bridge answer per tapped card. Typed follow-ups and un-marked highlights still fall through to the real key-required path. Keyed users skip all of this.
- `src/components/DriftPanel.tsx` — auto-sends a fresh keyless plain-drift on a pre-marked term (no chips to tap without a key); and **skips the "Choose a challenger" model picker** for the keyless Stress-test lens (it was a dead-end) so the canned answer shows directly.
- `src/App.tsx` — `hasOwnKey` memo; welcome example chips now show when `resumableTrees.length === 0 || !hasOwnKey`, so the free starters PERSIST across refreshes until the user adds their own key (previously they vanished after the first example created history).
- `src/hooks/useConnectionStatus.ts` callback in `App.tsx` — **no longer auto-opens Settings** when no key is present (it was reopening on every backoff poll, hijacking the screen); the Offline badge + Settings gear suffice.

**Tooltip fixes (two separate "jump"/clip bugs):**
- Welcome-card capability tooltip ("Branch without losing your place" etc.): moved from `sm:bottom-full` (above cards, clipped behind the top toolbar) to `sm:top-full` (below cards, over the starter-prompts area where there's reliably room), and removed its `animate-fade-up` — the keyframe's `transform: translateY` was clobbering the `-translate-x-1/2` centering, so it flashed right then jumped to center.
- Highlight **selection menu** (`SelectionTooltip.tsx`): swapped `animate-fade-up` for a new **opacity-only** `animate-fade-in` utility (`src/index.css`) so the inline `translate(-50%, …)` centering is never clobbered — fades in place, no jump. The touch action bar keeps `fade-up` (positioned via fixed offsets, no transform conflict).
- Selection menu **smart positioning**: `positionTooltip` used a hardcoded 96 px height estimate, but the full lens menu is ~250 px → a selection near the top "fit above", placed upward, and overflowed the top edge. Now it takes the real height, weighs room above vs below, flips to whichever fits (preferring above), and clamps Y so neither edge leaves the viewport. A `useLayoutEffect` measures the rendered menu and re-positions before paint, so it's correct at any selection position with no visible jump.

**Welcome-screen layout (from earlier in the session):** trimmed spacing so hero + 3 cards + all 4 example chips fit in one view on a ~800–900 px laptop without scrolling.

**PR / dependency housekeeping:**
- Verified-then-merged 6 green PRs (test-merged into `main` locally, ran tsc/build/tests on each): #11/#12/#13 (GitHub Actions bumps — checkout/gitleaks/setup-node), #30 (minor-and-patch group, 19 pkgs), #18 (globals 16→17), #15 (react-syntax-highlighter 15→16, runtime).
- **ESLint 10 upgrade** (resolves Dependabot #16/#17 which were failing CI on a peer conflict): bumped eslint ^10.5.0, @eslint/js ^10.0.1, **eslint-plugin-react-hooks ^7.1.1** (v5 doesn't support eslint 10 → `npm ci` ERESOLVE was the real failure), and adapted `eslint.config.js` (v7's `recommended-latest` is a legacy plugins-as-array config that eslint 10 flat config rejects — registered the plugin directly + kept the two classic rules). Closed #16/#17 as superseded.
- **Held PR #31** ("Onboarding wow + key-at-need, and clearer save actions"): technically green, but it's a *parallel implementation of this session's free-examples feature* — it seeds a full pre-baked sample exploration on first run (same Rome example, "no key needed" banner, drift terms, Drift Map, Synthesis, "Clear sample" button) plus an `ApiKeyPrompt` modal. Left open for a deliberate reconciliation (suggestion: keep its `ApiKeyPrompt` "key-at-need" modal, drop the duplicate seeded sample). Two failing eslint majors aside, this is the only open PR.

**Verification:** every step verified live on the keyless production build via the preview harness (example → canned root + dotted terms; each dotted term → all 6 lenses incl. Connect cards + a bridge; tooltips measured for no-clip/no-jump). `tsc -b` clean, `npm run build` green, 38/38 tests pass on `main`.

### 177. Challenge → Second opinion + Evidence citations (Jun 12)

Work across `src/lib/driftPanel.ts`, `src/components/` (DriftPanel, SelectionTooltip, DriftKnowledgeGraph, Settings, ShortcutsHelp), `src/services/` (driftMapExport), `src/App.tsx`, and tests. Committed + pushed to `main`. Build 60→61.

**Challenge lens → Second opinion:**
- Label renamed to "Second opinion" (tooltip shows "2nd Opinion"). The lens now frames its feedback as an independent second opinion rather than an adversarial challenge — a different qualified model weighs in with its verdict (agrees / agrees with caveats / disagrees), never manufacturing disagreement to seem rigorous.
- New prompt in `driftPanel.ts`: opens with one plain sentence verdict, then adds real value (strongest reason it holds, nuance the original missed, or fair single reason for disagreement). Explicitly forbidden from strawmanning or inventing objections. A confident "this is right" is a perfectly good second opinion.
- Trigger prefixes updated to "Second opinion on this" / "חוות דעת שנייה על זה" (old "Challenge this" kept for classification of existing chats).
- All UI surfaces updated: `SelectionTooltip.tsx` ("Another model weighs in"), `DriftPanel.tsx` lens bar ("2nd opinion"), drift header ("Second opinion from {model}"), Settings section ("Second-opinion model" + "Get an independent take…"), welcome tooltip, shortcuts help, knowledge graph, map export.

**Steelman lens removed:**
- Feature deleted from prompts, `BUILTIN_LENS_KEYS`, metadata, and tests. Evidence is now the only extra built-in beyond the signature four (Simplify/Deep dive/Connect/Second opinion).

**Evidence lens upgraded for citation quality:**
- Prompt now explicitly instructs use of Google Search grounding (already enabled by default in `gemini.ts`), preference for peer-reviewed sources (meta-analyses, Cochrane, RCTs, WHO/NIH/CDC, NEJM/Lancet/Nature) over blogs/press releases, and specific citations ("a 2019 Cochrane review", "Smith et al., NEJM 2021" with sample size and headline number).
- Evidence hierarchy made explicit: meta-analysis > RCT > observational > case report > expert opinion > anecdote.
- Fabricating citations stays hard-forbidden: if a source can't be verified, the model must describe the evidence type instead.

**UI fixes:**
- Template scaffold messages ("Second opinion on this: …") were rendering twice in the drift sidebar — once as the internal system message, once as the auto-sent user bubble. Render filter now skips scaffold-but-not-opener messages for system messages (`isDriftScaffoldText() && !isDriftOpenerText()`), keeping the plain-drift opener + Connect finder still visible.
- Numbered list layout bug: list-item renderer wrapped content in an inline `<span id=…>` (for scroll anchors on cross-reference links). When the item's content is block-level (a paragraph with bold header, common in numbered lists), the inline span forced an empty line at the start, orphaning the list marker ("1.") on its own line. Fix: moved anchor id directly onto the `<li>` element, drop the span. Works identically for scroll-to-anchor (ConversationScroller uses `getElementById`); worst in RTL but was subtle-wrong in English too.

### 176. Welcome-screen polish — fit-to-viewport + capability tooltips (Jun 11)

All work in `src/App.tsx` (empty-state block, ~line 2137), committed + pushed to `main` (`dc4a3aa` + this session's build commit). Build 59→60.

**No-scroll welcome screen:**
- Empty state changed from `pt-[22vh] pb-16` (which pushed content down and forced a scroll) to `min-h-full … justify-center py-5` so the hero, capability chips, and the resumable list are vertically centered and fit.
- On the welcome screen only, the scroll container drops its reserved input padding (`pt-6`→`pt-0`, bottom `9rem`→`6.5rem`) via `messages.length === 0` ternary — reclaims ~64px.
- Compacted the hero (icon 52→44, title 22→20px, tighter margins) and the "Pick up where you left off" cards: removed the standalone "Synthesize N drifts" button row, folded a tighter `✦ Synthesize N` action inline next to the term row (saves ~75px across 3 cards). Dropped the per-card GitBranch count chip.

**Three capability tooltips (replaces the single drift hint pill):**
- Row of three chips: **Highlight to drift** · **Shift lenses** · **Synthesize drifts** (icons: MousePointerClick / Layers / Sparkles; `Layers` newly imported).
- Custom glass popover replaces the native `title` bubble: dark-elevated/95, border, shadow, backdrop-blur, upward arrow. Opens **downward** (`top-full`) so it never clips against the header/scroll boundary above the chips.
- Each tooltip is a definition-style stack (no em dashes): bold name on its own line, quiet gloss subline beneath.
- **Drift** and **Synthesize** render as numbered steps (violet badges). Drift now covers the full loop: **Select → Branch → Push → Return** (Push = optionally fold a finding back into the chat).
- **Lenses** renders as a color-coded breakdown using each lens's real signature hue (matching the in-panel "View as" bar): Simplify amber / Deep dive blue / Connect cyan (accent-discovery) / Challenge rose, with a soft glow dot. Challenge gloss notes the **rival-AI-model second opinion** (the model-routing itself is another session's in-flight work, not yet on main).

**Build note:** `tsc --noEmit` passed but `tsc -b` (project mode, used by `npm run build`) flagged the points union type — fixed by giving the stepped points a uniform `{name,gloss,dot,text}` shape (violet). Production build clean: 302.03 kB JS / 127.00 kB CSS (gzip 87.76 / 18.69 kB).
...
- [ ] **TestFlight submission (build 60)** — Archive in Xcode GUI → upload to App Store Connect. Build number incremented 59→60, Capacitor synced, web assets ready. Requires: Xcode `ios/App/App.xcworkspace` → Product > Archive → Distribute App → TestFlight → Upload.
- [ ] **On-device pass — welcome screen (build 60)** — verify: empty state fits with NO scroll (even with 3 resumable cards); the three capability tooltips open downward without clipping; lens hues + 4-step drift loop read correctly; resumable cards' inline `✦ Synthesize N` still works.
- [ ] **On-device pass — cloud accounts (build 60)** — verify: (1) Sign in with Apple works on device; (2) Back up now / Restore / Sign out flow; (3) No Account UI visible when cloud disabled; (4) API keys NOT in backup payload (key-strip tests prove it, but verify on device).
- [ ] **On-device pass — UI polish (build 60)** — verify: arc label reads "related" (not "by field"); sidebar shows exactly one blank "New Chat" (no dupes); no "reopen last drift" chip in header.
